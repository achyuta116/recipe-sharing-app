    ...
    return (
        <div className='flex items-center w-full text-lg font-semibold h-14 shadow'>
            ...
            {!user?.token && <NavLink to='/login'
                className='w-min py-1 px-3 rounded-full bg-yellow-400 grid place-items-center ml-auto mr-1 hover:bg-yellow-500'>Login</NavLink>}
            {user?.token && <NavLink to='/'
                className='w-min py-1 px-3 rounded-full bg-yellow-400 grid place-items-center ml-auto mr-1 hover:bg-yellow-500' onClick={() => logout()}>Logout</NavLink>}
        </div>
    )
}
